...
class boncdeslectCtrl extends Controller {
			...
			if ($element) {
				...
				if ($groupes) {
					...
					foreach ($groupes as $k => $groupe) {
						$groupe['produits'] = $this->boncde_poste->query("SELECT P.lb_produit, P.sk_produit, B.qt_produit, B.co_produit FROM produit_had P LEFT JOIN boncde_poste B ON P.sk_produit = B.sk_produit and numcde='" . $numcde . "' WHERE lb_hierachie='" . $groupe['lb_hierachie'] . "' AND P.sk_client=:sk_client order by P.lb_classement,P.lb_produit ", array('sk_client' => auth::$auth ['sk_client']));
