...
<form name="editer" method="post" action="/boncdes/enregistrer">
							...
							<tbody >
								{foreach $groupes as $k => $groupe}
									...
									{foreach $groupe.produits as $k => $produit}	
										<tr>
											<td style="width:500px; padding-left:25px; size:8px">{$produit.lb_produit}</td>
											<td><input name="commentaire[{$produit.sk_produit}]" class="sk_produit" type="text" value="{$commentaire[$produit.sk_produit]}" style="width:300px " tabindex="-1" {if $element.datfinhad NE 0} readonly {/if}/></td>
											<td style="width:100px "> <b>{if $produit.sk_produit >10 || $produit.sk_produit <1  }
																		{$produit.sk_produit|replace:'FKL55651':'FKL5565'|replace:'FKL55652':'FKL5565'} 
																		{/if}</b></td>				
											
											
											
											<td><input name="quantite[{$produit.sk_produit}]" class="sk_produit {if $element.datfinhad EQ 0} spinner{/if}" type="text" value="{$quantite[$produit.sk_produit]}" style="width:50px " tabindex="-1" {if $element.datfinhad NE 0} readonly {/if} /></td>
	...
	<input name="btn_visualiser"  type="submit" class="boutton right" value="VISUALISER" />
	</span>
	<span {if $element.datfinhad NE 0} hidden {/if}>
